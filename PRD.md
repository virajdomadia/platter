...
- **PostGIS on Neon** (`create extension postgis`): `restaurants.loc`, `addresses.loc`, `rider_positions.loc` are `geography(Point, 4326)` with GIST. Restaurants for a pin = `ST_DWithin(loc, pin, 5000)` ordered by `ST_Distance`; nearest available rider = KNN `loc <-> restaurant.loc`.
