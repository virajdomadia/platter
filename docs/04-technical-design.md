...
3. **Tick** every 1 s (v3: on `NOTIFY`, with a 5-s safety poll): `advance_simulation(scope)`; `select … from order_events where seq > $seq and <scope> order by seq`; positions `where recorded_at > $pos_ms and rider in <scope>` (sim riders: computed, not read); `eta` recomputed when a status or a position changed; `yield` each as `id: {seq}:{pos_ms}\nevent: {kind}\ndata: {json}\n\n`. A `: ping` comment every 15 s keeps proxies open.
...
**Rider positions:** `POST /rider/position { lat, lng, heading, speed_mps, at }` → `rider_positions` upsert (`recorded_at = greatest(existing, at)`) + `rider_track` insert when the rider is on a job; a fix older than the stored one is ignored; a fix > 200 m from the previous within 3 s is clamped (GPS jump). Positions are pushed to the order stream of the rider's current order(s), the restaurant stream (distance to the restaurant while *to_restaurant*), and the admin stream (all riders every 2 s).
...
- **Position along a leg** (`services/sim.py`): decode once, cumulative haversine lengths, `progress = clamp((now − started_at) / duration_s)`, target distance = `progress × total`, interpolate on the segment; heading = bearing of that segment. Pure Python, no shapely.
...
- **Delivery fee:** `distance_m` = the restaurant → drop OSRM distance at quote time (fallback straight × 1.3); fee = 2500 + 800 × max(0, ceil((distance_m − 2000) / 1000)) paise; radius check on the same distance (`> 5000` → 422 `out_of_range`).
...
update orders set status = to, {to}_at = now(); insert order_events (order_id, kind = to, actor_role, actor_id, payload);
...
- **Board** (`app/(restaurant)/restaurant/page.tsx`): columns driven by `useRestaurantStream`; a new `placed` event **prints a ticket** (the signature on this screen) and plays a chime (user-gesture-unlocked once); actions call `POST /orders/{id}/transition { to }`.
...
| Rider phone loses GPS / goes to background | No fixes → the customer marker stops; after 30 s without a fix the tracking page shows "Rider's signal is weak"; the ETA freezes; the admin flags the rider after 2 min |
| GPS jump | A fix > 200 m from the previous within 3 s is dropped (logged) |
...
- `test_quote_and_order`: quote == order totals; tampered prices ignored; unavailable item 422; out of range 422; fee at 1.9 / 2.1 / 4.0 km.
...
- `test_sim`: a sim rider on a 600-m leg at 22 km/h is at the end after 98 s (fake clock); arrival before `ready` waits; `ready` triggers pickup; delivery frees the rider and assigns the waiting order.
